...
///////////////////////////////////////////
int device_selected_sh_1;
int device_selected_sh_2;
      ...
      if (t.isWeekdaySelected(day_i))
      {
        if ((weekday() + dayadjustment_var) == day_i) // day selected
        {
          ...
          if (((d1_startseconds + additional_scheduling_intervals) >= nowseconds_var) & (nowseconds_var >= d1_startseconds))
             {if(device_selected_sh_1=1)
          {
            d1_state = HIGH;
            d1_update(); //send values to devices for update
            Blynk.virtualWrite(vir_d2_man_button_pin_1, d1_state);
            Blynk.notify(String(configStore.mspace_1)+" powered on ");
          }if(device_selected_sh_1=2)
          {
            d2_state = HIGH;
...
{
  case 1:
  {device_selected_sh_1=1;
    break;
  }
  case 2:
  {device_selected_sh_1=2;
    break;
  }
...
{
  case 1:
  {device_selected_sh_2=1;
    break;
  }
  case 2:
  {device_selected_sh_2=2;
    break;
  }
