...
<form method='get' action='config'>
  ...
  <label>equipment_name: </label><input type="text" name="equipment_name" length=32><br/>
...
void enterConfigMode()
{
  ...
  byte mac[6];
  WiFi.macAddress(mac);
  String HOTSPOT_SSID_STRING=String(PRODUCT_WIFI_SSID)+String("_")+String(mac[0],HEX)+String(mac[1],HEX)+String(mac[2],HEX)+String(mac[3],HEX)+String(mac[4],HEX)+String(mac[5],HEX);
  ...
  DEBUG_PRINT(String("MAC ID: ") +  String(WiFi.macAddress()));
  ...
  server.on("/config", []() {
    ...
    String equip_name  = server.arg("equipment_name");
   if (equip_name == "") {
      equip_name = "smart_switch";
    }
    String content;
    unsigned statusCode;
    ...
    if (token.length() == 32 && ssid.length() > 0) {
      ...
      CopyString(equip_name, configStore.equip_name);
